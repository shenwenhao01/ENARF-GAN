...
class NeRF(nn.Module):
    ...
    def backbone_(self, p, z=None, j=None, bone_length=None, ray_direction=None):
        ...
        def clac_p_and_length_feature(p, bone_length):      # occupancy net(selector)
            ...
            net = self.fc_p(encoded_p)              # group=1 时 Conv1d 相当于全连接层
    ...
    def coarse_to_fine_sample(self, image_coord: torch.tensor, pose_to_camera: torch.tensor,
        ...
        with torch.no_grad():
            ...
            if inv_intrinsics.ndim == 2:
                ...
                sampled_camera_coord = torch.matmul(inv_intrinsics, image_coord)    # B*num_bone x 3 x n
            else:
        ...
        if pose_to_camera.requires_grad:
            ...
            fine_points = torch.matmul(R.permute(0, 1, 3, 2), fine_points - t).reshape(batchsize, num_bone * 3,
                                                                                       n * (Nc + Nf))
        return (
            fine_depth,  # Batch x 1 x 1 x n x Nc+Nf
    ...
    def render(self, image_coord: torch.tensor, pose_to_camera: torch.tensor, inv_intrinsics: torch.tensor,
        ...
        # image_coord: B x groups x 3 x n (sampled 2D image coordinate(齐次坐标),每个part重复一遍所有采样点位置)
        # camera_extrinsics: B x 4 x 4
        # camera_intrinsics: 3 x 3

        batchsize, num_bone, _, n = image_coord.shape

        fine_depth, fine_points, ray_direction = self.coarse_to_fine_sample(image_coord, pose_to_camera,        # TODO(swh): 理解关键的坐标系变换, 采样策略
                                                                            inv_intrinsics,
    ...
    def forward(self, batchsize, num_sample, sampled_img_coord, pose_to_camera, inv_intrinsics, z=None,
        ...
        :param batchsize:
        :param num_sample:
        :param sampled_img_coord: sampled 2D image coordinate(齐次坐标)
        :param pose_to_camera: camera coordinate of joint
        :param inv_intrinsics:
        :param z:
        :param world_pose: wold coordinate of joint
        :param bone_length:
        :param thres:
        :param render_scale:
        :param Nc:
